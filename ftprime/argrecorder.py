...
class ARGrecorder(OrderedDict):
    ...
    def add_individual(self,name,time,population=msprime.NULL_POPULATION,is_sample=False):
    ...
    def edgeset_table(self):
        ...
        for es in self.edgesets():
            table.add_row(
                left=es.left, right=es.right, parent=es.parent, children=es.children)
        return table
    ...
    def dump_sample_table(self, out):
        ...
        for idx in self:
            node = self[idx][0]
            if node.is_sample():
                out.write("{}\t{}\t{}\t{}\n".format(idx, node.flags, node.population, node.time))
    ...
    def add_samples(self,samples,length,populations=None):
        ...
        for k,parent in enumerate(samples):
            self.add_individual(k, time=0.0, population=populations[k], is_sample=True)
            self.add_record(
                    left=0.0,
                    right=length,
                    parent=parent,
